...
contract("APYMON", async (accounts) => {
  ...
  describe("Functionality", () => {
    ...
    it("Referral", async () => {
      ...
      // In this point, total supply should be 400 and 10% of 400(mean 20% of 200) will go to referral bonus
      const referralAmount = totalEthValue.div(new BigNumber(10));
      const bonusAmount = totalEthValue.div(new BigNumber(10));
      for (let i = 0; i < 10; i++) {
      ...
      await instanceMon.distributeBonus(0, 199);
      for (let i = 0; i < 200; i++) {
        await instancePack.openEgg(i, { from: accounts[i % 10] });
        ...
        assert.equal(new BigNumber(erc20Token.tokenBalances[0]).toString(10), '24000000000000000');
      }

      await instanceMon.distributeReferral(200, 399);
      await instanceMon.distributeBonus(200, 399);
      for (let j = 200; j < 400; j++) {
        await instancePack.openEgg(j, { from: accounts[j % 10] });
        assert.equal(erc20Token.addresses[0], '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2');
      ...
      assert.equal(totalEthValue.minus(referralAmount).minus(bonusAmount).toString(10), totalRaisedEth.toString(10));
    });
  });
});
